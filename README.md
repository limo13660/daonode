...
```bash
GOTOOLCHAIN=local GOEXPERIMENT=jsonv2 go test -tags with_quic ./... -count=1
GOTOOLCHAIN=local GOEXPERIMENT=jsonv2 go build -tags with_quic -trimpath \
  -ldflags "-X 'github.com/limo13660/daonode/cmd.version=v0.1.0' -s -w -buildid=" \
  -o daonode
```

如果系统默认 `go` 不是 `1.26.1`，先安装正确版本，或显式指定二进制：

```bash
make test GO=/path/to/go1.26.1/bin/go
```
